...
class hgcn(nn.Module):
    ...
    def __init__(self, in_channels=1024, out_channels=1, num_groups=8, num_clips=10,
        ...
        # Clip refinement
        # self.conv = nn.Conv1d(68, in_channels,1)
        ...
        self.prototype = nn.Embedding( num_clips, hidden_dim)
    ...
    def forward(self, x, return_feat = False):
        ...
        x = self.transformer.decoder(q, encode_x)
        #print(x.shape)
        x = x.permute(0,2,1)
        # print(x.shape)
        # x = self.conv(x)
        ...
        scores = mu + std * esp
        score = scores.mean(-1) * self.num_scenes
        out["output"] = score
        out['embed'] = None
        if return_feat:
            out['other'] = { "shot":shot, "scene":scene, "v":v,}
        return out
